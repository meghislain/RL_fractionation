...
class Grid:
    ...
    def __init__(self, xsize, ysize,  sources,
                 ...
                 alpha_tumor, beta_tumor, dose_bin_width,
                 oar=None):
        ...
        self.cumulative_dose = np.zeros((xsize, ysize))
                    ...
                    if len(res) > 2:  # If there are more than two arguments, a new cell must be created
                        if res[2] == 0: # Mitosis of a healthy cell
                            ...
                            if downhill is not None:
                                to_add.append((downhill[0], downhill[1], HealthyCell(4, average_healthy_glucose_absorption=self.average_healthy_glucose_absorption,
                            ...
                            if downhill is not None:
                                to_add.append((downhill[0], downhill[1], CancerCell(0, average_healthy_glucose_absorption=self.average_healthy_glucose_absorption,
                                             average_cancer_glucose_absorption=self.average_cancer_glucose_absorption,
                                             average_healthy_oxygen_consumption=self.average_healthy_oxygen_consumption,
                                             average_cancer_oxygen_consumption=self.average_cancer_oxygen_consumption,
                                             critical_glucose_level=self.critical_glucose_level,
    ...
    def calculate_DVH_voxel(self, name, mask_OAR, mask_PTV):
                            
        dose_bin_width = 0.2
    
        def DVH(cDose, mask):
            dose_values = cDose[mask].flatten() 
            total_volume = len(dose_values)
                
            # Specify the dose bins for the DVH
            max_dose = np.max(dose_values)
            print("max dose : ", max_dose)
            dose_bins = np.arange(0, max_dose + dose_bin_width, dose_bin_width)
                
            # Count the number of voxels receiving at least the dose corresponding to each bin
            volumes = []
            for bin in dose_bins:
                volume = np.sum(dose_values >= bin)
                volumes.append(volume)
            
            volumes = [vol / total_volume for vol in volumes]
            
            return dose_bins, volumes
        ...
        ax.set_title('Dose-Volume Histogram', fontsize = 55)
        plt.legend(fontsize=40)
        plt.xticks(fontsize=36)
        plt.yticks(fontsize=36)
        plt.savefig(f"DVH/plots/dose_volume_{name}.svg")
        plt.clf()
        
        from mpl_toolkits.axes_grid1 import make_axes_locatable
        
        cancer_cumulative_dose = np.zeros((self.xsize, self.ysize))
        healthy_cumulative_dose = np.zeros((self.xsize, self.ysize))
        cancer_cumulative_dose[mask_PTV] = self.cumulative_dose[mask_PTV]
        healthy_cumulative_dose[mask_OAR] = self.cumulative_dose[mask_OAR]
                
        fig, ax = plt.subplots(1, 2, figsize = (32,16))
        
        # Grid 1
        ax[0].set_title("Cumulative dose received \n by the target \n", fontsize=55, pad=10)
        ax[0].tick_params(labelsize=32)  # change grid tick size here
        div = make_axes_locatable(ax[0])
        cax = div.append_axes('right', '5%', '5%')
        im = ax[0].imshow(cancer_cumulative_dose, cmap='hot')
        cb = fig.colorbar(im, cax=cax)
        cb.ax.tick_params(labelsize=40)  # change colorbar tick size here
        
        # Grid 2
        ax[1].set_title("Cumulative dose received \n by the OAR \n", fontsize=55, pad=10)
        ax[1].tick_params(labelsize=32)  # change grid tick size here
        div = make_axes_locatable(ax[1])
        cax = div.append_axes('right', '5%', '5%')
        im = ax[1].imshow(healthy_cumulative_dose, cmap='hot')
        cb = fig.colorbar(im, cax=cax)
        cb.ax.tick_params(labelsize=40)  # change colorbar tick size here
    ...
    def irradiate(self, dose, center=None, rad=-1):
        ...
        mask_irr = np.zeros((self.xsize, self.ysize), dtype=bool) # No pixel has been irradiated yet.
        ...
        for i in range(self.xsize):
            for j in range(self.ysize):
                ...
                if dist < 3*radius:
                    ...
                    cell_dose = scale(radius, dist, multiplicator) * omf
                    if not mask_irr[i,j]:
                        self.cumulative_dose[i, j] += cell_dose
                        mask_irr[i,j] = True
                    for cell in self.cells[i, j]:
                        cell.radiate(cell_dose)
                    count = len(self.cells[i, j])
                    self.cells[i, j].delete_dead()
                    if len(self.cells[i, j]) < count:
                        self.add_neigh_count(i, j, len(self.cells[i, j]) - count)
